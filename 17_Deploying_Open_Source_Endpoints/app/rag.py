...
def _tiktoken_len(text: str) -> int:
    ...
    #tokens = tiktoken.encoding_for_model("BAAI/bge-large-en-v1.5").encode(text)
    #print(len(tokens))
    return len(tokens)
...
def _build_rag_graph(data_dir: str) -> "CompiledGraph":
    ...
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=300, chunk_overlap=0, length_function=_tiktoken_len
    )
    ...
    try:
        ...
        qdrant_vectorstore = Qdrant.from_documents(documents=chunks, embedding=embedding_model, location=":memory:"
    )
    except Exception as e:
        print(e)
        traceback.print_exc()
